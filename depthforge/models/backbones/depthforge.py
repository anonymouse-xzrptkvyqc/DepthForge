...
class DepthForge(nn.Module):
    ...
    def forward(
        ...
        delta_feat = self.forward_delta_feat(feats, tokens, depth_features, layer)
    ...
    def forward_delta_feat(
        self, feats: Tensor, tokens: Tensor, depth_features: Tensor, layers: int
    ) -> Tensor:

        attn = torch.einsum("nbc,mc->nbm", feats, tokens)
        depth_attn = torch.einsum("nbc,mc->nbm", depth_features, tokens)
        ...
        delta_f = torch.einsum(
            "nbm,mc->nbc", attn[:, :, 1:], self.mlp_token2feat(tokens[1:, :])
        )
...
@MODELS.register_module()
class DepthForgeReins(DepthForge):
